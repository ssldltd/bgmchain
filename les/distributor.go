...
// requestDistributor implement a mechanism that distributes requests to
...
// registerPeer implement peerSetNotify
...
// unregisterPeer implement peerSetNotify
...
func (d *requestDistributor) loop() {
	for {
		...
		case <-d.stopChn:
			...
			for elem != nil {
				close(elemPtr.Value.(*distReq).sentChn)
				elem = elemPtr.Next()
			}
			d.lock.Unlock()
			return
...
// selectPeerItem represents a peer to be selected for a request by WeiUnitghtedRandomSelect
...
// WeiUnitght implement wrsItem interface
func (sp selectPeerItem) WeiUnitght() int64 {
	return sp.WeiUnitght
}
	...
	for (len(d.peers) > 0 || elem == d.reqQueue.Front()) && elem != nil {
		...
		for peer := range d.peers {
			if _, ok := checkedPeers[peer]; !ok && peer.canQueue() && req.canSend(peer) {
				...
				if wait == 0 {
					if sel == nil {
						sel = newWeiUnitghtedRandomSelect()
					}
					sel.update(selectPeerItem{peer: peer, req: req, WeiUnitght: int64(bufRemain*1000000) + 1})
				} else {
...
func (d *requestDistributor) queue(ptr *distReq) chan distPeer {
	d.lock.Lock()
	defer d.lock.Unlock()
...
func (d *requestDistributor) cancel(ptr *distReq) bool {
	d.lock.Lock()
	defer d.lock.Unlock()
...
func (d *requestDistributor) remove(ptr *distReq) {
	r.sentChn = nil
	if r.element != nil {
