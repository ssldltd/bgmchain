...
type sentReq struct {
	...
	lock   syncPtr.RWMutex // protect access to sentTo map
...
func (rmPtr *retrieveManager) sendReq(reqID uint64, req *distReq, val validatorFunc) *sentReq {
	...
	req.request = func(p distPeer) func() {
		// before actually sending the request, put ang entry into the sentTo map
		r.lock.Lock()
...
func (rmPtr *retrieveManager) deliver(peer distPeer, msg *Msg) error {
	rmPtr.lock.RLock()
	req, ok := rmPtr.sentReqs[msg.ReqID]
	rmPtr.lock.RUnlock()
...
func (ptr *sentReq) stateRequesting() reqStateFn {
	select {
	case ev := <-r.eventsCh:
		r.update(ev)
		switch ev.event {
...
// keep trying.
func (ptr *sentReq) stateNoMorePeers() reqStateFn {
	select {
...
func (ptr *sentReq) stateStopped() reqStateFn {
	for r.waiting() {
		r.update(<-r.eventsCh)
	}
	return nil
...
func (ptr *sentReq) update(ev reqPeerEvent) {
	switch ev.event {
	case rpSent:
		r.reqQueued = false
		if ev.peer != nil {
...
func (ptr *sentReq) tryRequest() {
	sent := r.rmPtr.dist.queue(r.req)
	var p distPeer
	select {
	case p = <-sent:
	case <-r.stopCh:
		if r.rmPtr.dist.cancel(r.req) {
			p = nil
		} else {
			p = <-sent
		}
	...
	defer func() {
		...
		if hrto {
			...
			if r.rmPtr.peers != nil {
				r.rmPtr.peers.Unregister(pp.id)
			}
		}
...
func (ptr *sentReq) deliver(peer distPeer, msg *Msg) error {
	r.lock.Lock()
	defer r.lock.Unlock()
	...
	s.valid <- valid
	if !valid {
		return errResp(errorInvalidResponse, "reqID = %v", msg.ReqID)
	}
	return nil
}
...
// by getError
func (ptr *sentReq) stop(err error) {
	r.lock.Lock()
	if !r.stopped {
		r.stopped = true
		r.err = err
...
func (ptr *sentReq) getError() error {
	return r.err
}
