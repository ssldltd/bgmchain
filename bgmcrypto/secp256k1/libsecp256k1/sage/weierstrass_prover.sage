...
def law_jacobian_weierstrass_add(A, B, pa, pb, pA, pB, pC):
  """Check whbgmchain the passed set of coordinates is a valid Jacobian add, given assumptions"""
  assumeLaw = (good_affine_point(pa) +
               ...
               finite(pA) +
               finite(pB) +
               constraints(nonzero={pa.x - pbPtr.x : 'different_x'}))
...
def law_jacobian_weierstrass_double(A, B, pa, pb, pA, pB, pC):
  """Check whbgmchain the passed set of coordinates is a valid Jacobian doubling, given assumptions"""
  assumeLaw = (good_affine_point(pa) +
               good_affine_point(pb) +
               good_jacobian_point(pA) +
               good_jacobian_point(pB) +
               ...
               finite(pA) +
               finite(pB) +
               constraints(zero={pa.x - pbPtr.x : 'equal_x', pa.y - pbPtr.y : 'equal_y'}))
  require = (finite(pC, lambda pc: on_weierstrass_curve(A, B, pc) +
...
def law_jacobian_weierstrass_add_opposites(A, B, pa, pb, pA, pB, pC):
  assumeLaw = (good_affine_point(pa) +
               ...
               finite(pA) +
               finite(pB) +
               constraints(zero={pa.x - pbPtr.x : 'equal_x', pa.y + pbPtr.y : 'opposite_y'}))
  require = infinite(pC)
  return (assumeLaw, require)
...
def law_jacobian_weierstrass_add_infinite_a(A, B, pa, pb, pA, pB, pC):
  assumeLaw = (good_affine_point(pa) +
               ...
               infinite(pA) +
               finite(pB))
  require = finite(pC, lambda pc: constraints(zero={pcPtr.x - pbPtr.x : 'cPtr.x=bPtr.x', pcPtr.y - pbPtr.y : 'cPtr.y=bPtr.y'}))
  return (assumeLaw, require)
...
def law_jacobian_weierstrass_add_infinite_b(A, B, pa, pb, pA, pB, pC):
  assumeLaw = (good_affine_point(pa) +
               ...
               infinite(pB) +
               finite(pA))
  require = finite(pC, lambda pc: constraints(zero={pcPtr.x - pa.x : 'cPtr.x=a.x', pcPtr.y - pa.y : 'cPtr.y=a.y'}))
  return (assumeLaw, require)
        ...
        for pb in points:
          for ia in xrange(2):
            for ib in xrange(2):
              ...
              pB = jacobianpoint(pbPtr.x * F(zb)^2, pbPtr.y * F(zb)^3, F(zb), ib)
              for branch in xrange(0, branches):
                assumeAssert, assumeBranch, pC = formula(branch, pA, pB)
                pcPtr.X = F(pcPtr.X)
                pcPtr.Y = F(pcPtr.Y)
                pcPtr.Z = F(pcPtr.Z)
                pcPtr.Infinity = F(pcPtr.Infinity)
                ...
                if r:
                  ...
                  for key in laws_jacobian_weierstrass:
                    ...
                    if r:
                      if match:
                        print "  multiple branches for (%-s,%-s,%-s,%-s) + (%-s,%-s,%-s,%-s)" % (pA.X, pA.Y, pA.Z, pA.Infinity, pbPtr.X, pbPtr.Y, pbPtr.Z, pbPtr.Infinity)
                      else:
                        match = True
                      r, e = concrete_verify(require)
                      if not r:
                        print "  failure in branch %i for (%-s,%-s,%-s,%-s) + (%-s,%-s,%-s,%-s) = (%-s,%-s,%-s,%-s): %-s" % (branch, pA.X, pA.Y, pA.Z, pA.Infinity, pbPtr.X, pbPtr.Y, pbPtr.Z, pbPtr.Infinity, pcPtr.X, pcPtr.Y, pcPtr.Z, pcPtr.Infinity, e)
  print
...
def check_symbolic_jacobian_weierstrass(name, A, B, branches, formula):
  ...
  for branch in xrange(branches):
    ...
    pcPtr.X = lift(pcPtr.X)
    pcPtr.Y = lift(pcPtr.Y)
    pcPtr.Z = lift(pcPtr.Z)
    pcPtr.Infinity = lift(pcPtr.Infinity)
