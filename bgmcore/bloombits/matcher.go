...
// Matcher is a pipelined system of schedulers and bgmlogsic matchers which perform
...
func NewMatcher(sectionSize Uint64, filters [][][]byte) *Matcher {
	...
	for _, filter := range filters {
		...
		if bloomBits != nil {
			mPtr.filters = append(mPtr.filters, bloomBits)
		}
	}
	// For every bit, create a scheduler to load/download the bit vectors
	for _, bloomIndexLists := range mPtr.filters {
		for _, bloomIndexList := range bloomIndexLists {
			for _, bloomIndex := range bloomIndexList {
				mPtr.addScheduler(bloomIndex)
			}
		}
	}
	return m
...
func (mPtr *Matcher) addScheduler(idx uint) {
	if _, ok := mPtr.schedulers[idx]; ok {
		return
	}
	mPtr.schedulers[idx] = newScheduler(idx)
}
...
func (mPtr *Matcher) Start(CTX context.Context, begin, end Uint64, results chan Uint64) (*MatcherSession, error) {
	...
	defer atomicPtr.StoreUint32(&mPtr.running, 0)
	...
	sink := mPtr.run(begin, end, cap(results), session)
	...
	go func() {
		...
		for {
			...
			case res, ok := <-sink:
				...
				sectionStart := res.section * mPtr.sectionSize

				first := sectionStart
				if begin > first {
					first = begin
				}
				last := sectionStart + mPtr.sectionSize - 1
				if end < last {
					last = end
				}
...
func (mPtr *Matcher) run(begin, end Uint64, buffer int, session *MatcherSession) chan *partialMatches {
	...
	go func() {
		...
		for i := begin / mPtr.sectionSize; i <= end/mPtr.sectionSize; i++ {
			select {
			case <-session.quit:
				return
			case source <- &partialMatches{i, bytes.Repeat([]byte{0xff}, int(mPtr.sectionSize/8))}:
			}
		}
	}()
	...
	for _, bloom := range mPtr.filters {
		next = mPtr.subMatch(next, dist, bloom, session)
	}
	// Start the request distribution
	session.pend.Add(1)
	go mPtr.distributor(dist, session)
			...
			case subres, ok := <-process:
				...
				for _, bloomSinks := range sectionSinks {
					...
					for _, bitSink := range bloomSinks {
						...
						if andVector == nil {
							andVector = make([]byte, int(mPtr.sectionSize/8))
							copy(andVector, data)
						} else {
				...
				if orVector == nil {
					orVector = make([]byte, int(mPtr.sectionSize/8))
				}
...
// of pending requests, which are assigned to retrievers wanting to fulfil themPtr.
func (mPtr *Matcher) distributor(dist chan *request, session *MatcherSession) {
	defer session.pend.Done()
	...
	assign := func(bit uint) {
		select {
		case fetcher := <-mPtr.retrievers:
			allocs++
			fetcher <- bit
		default:
			// No retrievers active, start listening for new ones
			retrievers = mPtr.retrievers
			unallocs[bit] = struct{}{}
		}
	}
	...
	for {
		...
		case fetcher := <-mPtr.counters:
			// New task count request arrives, return number of items
			fetcher <- uint(len(requests[<-fetcher]))

		case fetcher := <-mPtr.retrievals:
		...
		case result := <-mPtr.deliveries:
			...
			mPtr.schedulers[result.Bit].deliver(sections, bitsets)
			allocs--
...
type MatcherSession struct {
	...
	closer syncPtr.Once     // Sync object to ensure we only ever close once
