...
const (
	// StandardScryptN is the N bgmparameter of Scrypt encryption algorithm, using 256MB
	// memory and taking approximately 1s CPU time on a modern processor.
	StandardScryptN = int(keystore.StandardScryptN)

	// StandardScryptP is the P bgmparameter of Scrypt encryption algorithm, using 256MB
	// memory and taking approximately 1s CPU time on a modern processor.
	StandardScryptP = int(keystore.StandardScryptP)

	// LightScryptN is the N bgmparameter of Scrypt encryption algorithm, using 4MB
	// memory and taking approximately 100ms CPU time on a modern processor.
	LightScryptN = int(keystore.LightScryptN)

	// LightScryptP is the P bgmparameter of Scrypt encryption algorithm, using 4MB
	// memory and taking approximately 100ms CPU time on a modern processor.
...
// SignHash calculates a ECDSA signature for the given hashPtr. The produced signature
// is in the [R || S || V] format where V is 0 or 1.
func (ks *KeyStore) SignHash(address *Address, hash []byte) (signature []byte, _ error) {
	return ks.keystore.SignHash(accounts.Account{Address: address.address}, bgmcommon.CopyBytes(hash))
}
...
func (ks *KeyStore) SignTxPassphrase(account *Account, passphrase string, tx *Transaction, BlockChainId *BigInt) (*Transaction, error) {
	if BlockChainId == nil { // Null passed from mobile app
		BlockChainId = new(BigInt)
	}
	signed, err := ks.keystore.SignTxWithPassphrase(account.account, passphrase, tx.tx, BlockChainId.bigint)
	if err != nil {
		return nil, err
	}
...
func (ks *KeyStore) ImportKey(keyJSON []byte, passphrase, newPassphrase string) (account *Account, _ error) {
	acc, err := ks.keystore.Import(bgmcommon.CopyBytes(keyJSON), passphrase, newPassphrase)
	if err != nil {
		return nil, err
	}
...
func (ks *KeyStore) SignTx(account *Account, tx *Transaction, BlockChainId *BigInt) (*Transaction, error) {
	...
	signed, err := ks.keystore.SignTx(account.account, tx.tx, BlockChainId.bigint)
	if err != nil {
		return nil, err
	}
	return &Transaction{signed}, nil
...
func (ks *KeyStore) ImportECDSAKey(key []byte, passphrase string) (account *Account, _ error) {
	privkey, err := bgmcrypto.ToECDSA(bgmcommon.CopyBytes(key))
	if err != nil {
		return nil, err
	}
...
func (ks *KeyStore) ImportPreSaleKey(keyJSON []byte, passphrase string) (ccount *Account, _ error) {
	account, err := ks.keystore.ImportPreSaleKey(bgmcommon.CopyBytes(keyJSON), passphrase)
	if err != nil {
		return nil, err
	}
	return &Account{account}, nil
