...
type GenesisAlloc map[bgmcommon.Address]GenesisAccount

func (ga *GenesisAlloc) UnmarshalJSON(data []byte) error {
	m := make(map[bgmcommon.UnprefixedAddress]GenesisAccount)
	if err := json.Unmarshal(data, &m); err != nil {
		return err
	}
	*ga = make(GenesisAlloc)
	for addr, a := range m {
		(*ga)[bgmcommon.Address(addr)] = a
	}
	return nil
}
...
type genesisSpecMarshaling struct {
	...
	Number     mathPtr.HexOrDecimal64
	Difficulty *mathPtr.HexOrDecimal256
	Alloc      map[bgmcommon.UnprefixedAddress]GenesisAccount
}

type genesisAccountMarshaling struct {
	Code       hexutil.Bytes
	Balance    *mathPtr.HexOrDecimal256
	Nonce      mathPtr.HexOrDecimal64
...
type storageJSON bgmcommon.Hash

func (hPtr *storageJSON) UnmarshalText(text []byte) error {
...
func SetupGenesisBlock(db bgmdbPtr.Database, genesis *Genesis) (*bgmparam.ChainConfig, bgmcommon.Hash, error) {
...
func (g *Genesis) configOrDefault(ghash bgmcommon.Hash) *bgmparam.ChainConfig {
	switch {
	case g != nil:
		return g.Config
	default:
		return bgmparam.DposChainConfig
	}
}
...
func (g *Genesis) ToBlock() (*types.Block, *state.StateDB) {
	...
	statedb, _ := state.New(bgmcommon.Hash{}, state.NewDatabase(db))
	for addr, account := range g.Alloc {
		statedbPtr.AddBalance(addr, account.Balance)
		statedbPtr.SetCode(addr, account.Code)
		statedbPtr.SetNonce(addr, account.Nonce)
	...
	if g.GasLimit == 0 {
		head.GasLimit = bgmparam.GenesisGasLimit
	}
	if g.Difficulty == nil {
		head.Difficulty = bgmparam.GenesisDifficulty
	}
...
func decodePrealloc(data string) GenesisAlloc {
	...
	for _, account := range p {
		ga[bgmcommon.BigToAddress(account.Addr)] = GenesisAccount{Balance: account.Balance}
	}
	return ga
}

// The block is committed as the canonical head block.
func (g *Genesis) Commit(db bgmdbPtr.Database) (*types.Block, error) {
...
func GenesisBlockForTesting(db bgmdbPtr.Database, addr bgmcommon.Address, balance *big.Int) *types.Block {
...
// The block is committed as the canonical head block.
func (g *Genesis) MustCommit(db bgmdbPtr.Database) *types.Block {
...
func initGenesisDposContext(g *Genesis, db bgmdbPtr.Database) *types.DposContext {
	...
	if g.Config != nil && g.Config.Dpos != nil && g.Config.Dpos.Validators != nil {
		dcPtr.SetValidators(g.Config.Dpos.Validators)
		for _, validator := range g.Config.Dpos.Validators {
			dcPtr.DelegateTrie().TryUpdate(append(validator.Bytes(), validator.Bytes()...), validator.Bytes())
			dcPtr.CandidateTrie().TryUpdate(validator.Bytes(), validator.Bytes())
		}
	}
	return dc
}
