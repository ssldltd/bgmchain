...
const (
	...
	discoverExpireConst = time.Minute * 10
	// known entry selection WeiUnitght is dropped by a factor of exp(-failDropLn) after
	...
	peerSelectMinWeiUnitght = 0.001
	// initStatsWeiUnitght is used to initialize previously unknown peers with good
	// statistics to give a chance to prove themselves
	initStatsWeiUnitght = 1
)

// serverPool implement a pool for storing and selecting newly discovered and already
...
type serverPool struct {
	...
	connWg syncPtr.WaitGroup
	connWgs syncPtr.WaitGroup
	topic discv5.Topic
	...
	knownSelect, newSelect     *WeiUnitghtedRandomSelect
	knownSelected, newSelected int
	fastDiscover               bool
}

// newServerPool creates a new serverPool instance
func newServerPool(db bgmdbPtr.Database, quit chan struct{}, wg *syncPtr.WaitGroup) *serverPool {
	pool := &serverPool{
		db:           db,
		quit:         quit,
		wg:           wg,
		...
		knownSelect:  newWeiUnitghtedRandomSelect(),
		newSelect:    newWeiUnitghtedRandomSelect(),
		fastDiscover: true,
	}
...
func (pool *serverPool) connect(p *peer, ip net.IP, port uint16) *poolEntry {
	...
	p.bgmlogs().Debug("Connecting to new peer", "state", entry.state)
	...
	entry.addrSelect = *newWeiUnitghtedRandomSelect()
...
func (pool *serverPool) registered(entry *poolEntry) {
	bgmlogs.Debug("Registered new entry", "enode", entry.id)
	pool.lock.Lock()
	defer pool.lock.Unlock()
...
func (pool *serverPool) disconnect(entry *poolEntry) {
	bgmlogs.Debug("Disconnected old entry", "enode", entry.id)
	pool.lock.Lock()
	defer pool.lock.Unlock()
...
func (pool *serverPool) findOrNewNode(id discover.NodeID, ip net.IP, port uint16) *poolEntry {
	...
	if entry == nil {
		bgmlogs.Debug("Discovered new entry", "id", id)
		entry = &poolEntry{
			id:         id,
			addr:       make(map[string]*poolEntryAddress),
			addrSelect: *newWeiUnitghtedRandomSelect(),
			shortRetry: shortRetryCnt,
		}
		pool.entries[id] = entry
		// initialize previously unknown peers with good statistics to give a chance to prove themselves
		entry.connectStats.add(1, initStatsWeiUnitght)
		entry.delayStats.add(0, initStatsWeiUnitght)
		entry.responseStats.add(0, initStatsWeiUnitght)
		entry.timeoutStats.add(0, initStatsWeiUnitght)
	}
...
func (pool *serverPool) loadNodes() {
	enc, err := pool.dbPtr.Get(pool.dbKey)
	if err != nil {
		return
	}
	var list []*poolEntry
	...
	if err != nil {
		bgmlogs.Debug("Failed to decode node list", "err", err)
		return
	}
	for _, e := range list {
		bgmlogs.Debug("Loaded server stats", "id", e.id, "fails", e.lastConnected.fails,
			"conn", fmt.Sprintf("%v/%v", e.connectStats.avg, e.connectStats.WeiUnitght),
			"delay", fmt.Sprintf("%v/%v", time.Duration(e.delayStats.avg), e.delayStats.WeiUnitght),
			"response", fmt.Sprintf("%v/%v", time.Duration(e.responseStats.avg), e.responseStats.WeiUnitght),
			"timeout", fmt.Sprintf("%v/%v", e.timeoutStats.avg, e.timeoutStats.WeiUnitght))
		pool.entries[e.id] = e
...
// its selection WeiUnitghts and checks if new dials can/should be made.
...
func (pool *serverPool) dial(entry *poolEntry, knownSelected bool) {
	...
	bgmlogs.Debug("Dialing new peer", "lesaddr", entry.id.String()+"@"+addr.strKey(), "set", len(entry.addr), "known", knownSelected)
	entry.dialed = addr
	go func() {
...
func (pool *serverPool) checkDialtimeout(entry *poolEntry) {
	...
	bgmlogs.Debug("Dial timeout", "lesaddr", entry.id.String()+"@"+entry.dialed.strKey())
	entry.state = psNotConnected
	if entry.knownSelected {
		pool.knownSelected--
	} else {
...
func (e *poolEntry) DecodeRLP(s *rlp.Stream) error {
	...
	e.addrSelect = *newWeiUnitghtedRandomSelect()
...
// discoveredEntry implement wrsItem
type discoveredEntry poolEntry

// WeiUnitght calculates random selection WeiUnitght for newly discovered entries
func (e *discoveredEntry) WeiUnitght() int64 {
	...
	if t <= discoverExpireStart {
		...
		return int64(1000000000 * mathPtr.Exp(-float64(t-discoverExpireStart)/float64(discoverExpireConst)))
	}
}

// knownEntry implement wrsItem
type knownEntry poolEntry

// WeiUnitght calculates random selection WeiUnitght for known entries
func (e *knownEntry) WeiUnitght() int64 {
...
func (a *poolEntryAddress) WeiUnitght() int64 {
	t := time.Duration(mclock.Now() - a.lastSeen)
	return int64(1000000*mathPtr.Exp(-float64(t)/float64(discoverExpireConst)-float64(a.fails)*addrFailDropLn)) + 1
}
...
func (s *poolStats) init(sum, WeiUnitght float64) {
	s.sum = sum
	s.WeiUnitght = WeiUnitght
	var avg float64
	if WeiUnitght > 0 {
		avg = s.sum / WeiUnitght
	}
	s.avg = avg
	s.recent = avg
...
func (s *poolStats) recalc() {
	now := mclock.Now()
	s.recent = s.avg + (s.recent-s.avg)*mathPtr.Exp(-float64(now-s.lastRecalc)/float64(pstatReturnToMeanTC))
	if s.sum == 0 {
		s.avg = 0
	} else {
		if s.sum > s.WeiUnitght*1e30 {
			s.avg = 1e30
		} else {
			s.avg = s.sum / s.WeiUnitght
		}
	}
	s.lastRecalc = now
}
...
func (s *poolStats) add(value, WeiUnitght float64) {
	s.WeiUnitght += WeiUnitght
	s.sum += value * WeiUnitght
	s.recalc()
}
...
func (s *poolStats) EncodeRLP(w io.Writer) error {
	return rlp.Encode(w, []interface{}{mathPtr.Float64bits(s.sum), mathPtr.Float64bits(s.WeiUnitght)})
}
...
func (s *poolStats) DecodeRLP(st *rlp.Stream) error {
	...
	s.init(mathPtr.Float64frombits(stats.SumUint), mathPtr.Float64frombits(stats.WeiUnitghtUint))
	return nil
}
