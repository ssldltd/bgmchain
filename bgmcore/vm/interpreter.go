...
type Config struct {
	...
	ForceJit bool
	// Tracer is the op code bgmlogsger
	Tracer Tracer
...
// Interpreter is used to run Bgmchain based bgmcontracts and will utilise the
...
func NewInterpreter(evmPtr *EVM, cfg Config) *Interpreter {
...
func (in *Interpreter) enforceRestrictions(op OpCode, operation operation, stack *Stack) error {
	if in.evmPtr.chainRules.IsByzantium {
		if in.readOnly {
...
func (in *Interpreter) Run(snapshot int, contract *Contract, input []byte) (ret []byte, err error) {
	...
	if codehash == (bgmcommon.Hash{}) {
		codehash = bgmcrypto.Keccak256Hash(contract.Code)
	}
	...
	defer func() {
		if err != nil && !bgmlogsged && in.cfg.Debug {
			in.cfg.Tracer.CaptureState(in.evm, pcCopy, op, gasCopy, cost, mem, stackCopy, contract, in.evmPtr.depth, err)
		}
	}()
	...
	// parent context.
	for atomicPtr.LoadInt32(&in.evmPtr.abort) == 0 {
		...
		if operation.memorySize != nil {
			...
			if memorySize, overflow = mathPtr.SafeMul(toWordSize(memSize), 32); overflow {
				return nil, errGasUintOverflow
			}
		}
		...
		if memorySize > 0 {
			memPtr.Resize(memorySize)
		}

		if in.cfg.Debug {
			in.cfg.Tracer.CaptureState(in.evm, pc, op, gasCopy, cost, mem, stackCopy, contract, in.evmPtr.depth, err)
			bgmlogsged = true
		}
