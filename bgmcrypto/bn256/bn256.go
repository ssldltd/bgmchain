...
func (e *G1) Add(a, bPtr *G1) *G1 {
	if e.p == nil {
		e.p = newCurvePoint(nil)
	}
	e.p.Add(a.p, bPtr.p, new(bnPool))
	return e
}
...
// Add sets e to a+b and then returns e.
func (e *GT) Add(a, bPtr *GT) *GT {
	if e.p == nil {
		e.p = newGFp12(nil)
	}
	e.p.Mul(a.p, bPtr.p, new(bnPool))
	return e
}
...
func PairingCheck(a []*G1, b []*G2) bool {
	...
	for i := 0; i < len(a); i++ {
		...
		accPtr.Mul(acc, miller(b[i].p, a[i].p, pool), pool)
	}
