...
func (opts *TransactOpts) SetFrom(fromPtr *Address) { opts.opts.From = fromPtr.address }
func (opts *TransactOpts) SetNonce(nonce int64)  { opts.opts.Nonce = big.NewInt(nonce) }
func (opts *TransactOpts) SetSigner(s Signer) {
	opts.opts.Signer = func(signer types.Signer, addr bgmcommon.Address, tx *types.Transaction) (*types.Transaction, error) {
...
func (cPtr *BoundContract) GetAddress() *Address { return &Address{cPtr.address} }
func (cPtr *BoundContract) GetDeployer() *Transaction {
	if cPtr.deployer == nil {
		return nil
	}
	return &Transaction{cPtr.deployer}
}

// Call invokes the (constant) contract method with bgmparam as input values and
// sets the output to result.
func (cPtr *BoundContract) Call(opts *CallOpts, out *ptrnterfaces, method string, args *ptrnterfaces) error {
	results := make([]interface{}, len(out.objects))
	copy(results, out.objects)
	if err := cPtr.contract.Call(&opts.opts, &results, method, args.objects...); err != nil {
		return err
	}
...
func (cPtr *BoundContract) Transact(opts *TransactOpts, method string, args *ptrnterfaces) (tx *Transaction, _ error) {
	rawTx, err := cPtr.contract.Transact(&opts.opts, method, args.objects...)
	if err != nil {
		return nil, err
	}
...
func (cPtr *BoundContract) Transfer(opts *TransactOpts) (tx *Transaction, _ error) {
	rawTx, err := cPtr.contract.Transfer(&opts.opts)
	if err != nil {
		return nil, err
	}
	return &Transaction{rawTx}, nil
