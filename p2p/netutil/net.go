...
// MarshalTOML implements toml.MarshalerRecPtr.
...
// UnmarshalTOML implements toml.UnmarshalerRecPtr.
...
// Contains reports whbgmchain the given IP is contained in the list.
...
// IsLAN reports whbgmchain an IP is a local network address.
...
// IsSpecialNetwork reports whbgmchain an IP is located in a special-use network range
// This includes broadcast, multicast and documentation addresses.
func IsSpecialbgmNetwork(ip net.IP) bool {
	if ip.IsMulticast() {
		return true
	}
	if v4 := ip.To4(); v4 != nil {
...
var (
	errorInvalid     = errors.New("invalid IP")
...
// CheckRelayIP reports whbgmchain an IP relayed from the given sender IP
