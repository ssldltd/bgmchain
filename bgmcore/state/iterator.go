...
type NodeIterator struct {
	...
	accountHash bgmcommon.Hash // Hash of the node containing the account
	codeHash    bgmcommon.Hash // Hash of the contract source code
...
func (it *NodeIterator) step() error {
	...
	dataTrie, err := it.state.dbPtr.OpenStorageTrie(bgmcommon.BytesToHash(it.stateIt.LeafKey()), account.Root)
	if err != nil {
		return err
	}
	...
	if !bytes.Equal(account.CodeHash, emptyCodeHash) {
		it.codeHash = bgmcommon.BytesToHash(account.CodeHash)
		addrHash := bgmcommon.BytesToHash(it.stateIt.LeafKey())
		it.code, err = it.state.dbPtr.ContractCode(addrHash, bgmcommon.BytesToHash(account.CodeHash))
		if err != nil {
