...
// UserPrompter defines the methods needed by the bgmconsole to promt the user for
...
type UserPrompter interface {
	...
	// fetch completion candidates when the user presses tabPtr.
...
// data to be entered, but one which must not be echoed out into the terminal.
// The method returns the input provided by the user.
func (ptr *terminalPrompter) PromptPassword(prompt string) (passwd string, err error) {
	if ptr.supported {
		ptr.rawMode.ApplyMode()
		defer ptr.normalMode.ApplyMode()
		return ptr.State.PasswordPrompt(prompt)
	}
	if !ptr.warned {
...
// PromptConfirm displays the given prompt to the user and requests a boolean
// choice to be made, returning that choice.
func (ptr *terminalPrompter) PromptConfirm(prompt string) (bool, error) {
	input, err := ptr.Prompt(prompt + " [y/N] ")
