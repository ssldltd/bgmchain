...
// ErrorTrezorPINNeeded is returned if opening the trezor requires a PIN code. In
...
type trezorDriver struct {
	...
	pinwait bool          // Flags whbgmchain the device is waiting for PIN entry
	failure error         // Any failure that would make the device unusable
	bgmlogs     bgmlogs.bgmlogsger    // Contextual bgmlogsger to tag the trezor with its id
}
...
// Status implements accounts.Wallet, always whbgmchain the Trezor is opened, closed
// or whbgmchain the Bgmchain app was not started on it.
...
func newTrezorDriver(bgmlogsger bgmlogs.bgmlogsger) driver {
	return &trezorDriver{
		bgmlogs: bgmlogsger,
	}
}
...
func (w *trezorDriver) Derive(path accounts.DerivationPath) (bgmcommon.Address, error) {
	return w.trezorDerive(path)
}


// trezorDerive sends a derivation request to the Trezor device and returns the
// Bgmchain address located on that pathPtr.
func (w *trezorDriver) trezorDerive(derivationPath []uint32) (bgmcommon.Address, error) {
	address := new(trezor.BgmchainAddress)
	if _, err := w.trezorExchange(&trezor.BgmchainGetAddress{AddressN: derivationPath}, address); err != nil {
		return bgmcommon.Address{}, err
	}
	return bgmcommon.BytesToAddress(address.GetAddress()), nil
}
...
func (w *trezorDriver) trezorExchange(req proto.Message, results ...proto.Message) (int, error) {
	...
	for len(payload) > 0 {
		...
		w.bgmlogs.Trace("Data chunk sent to the Trezor", "chunk", hexutil.Bytes(chunk))
	...
	if kind == uint16(trezor.MessagesType_MessagesType_Failure) {
	...
	if kind == uint16(trezor.MessagesType_MessagesType_ButtonRequest) {
...
func (w *trezorDriver) SignTx(path accounts.DerivationPath, tx *types.Transaction, chainID *big.Int) (bgmcommon.Address, *types.Transaction, error) {
	if w.device == nil {
...
func (w *trezorDriver) trezorSign(derivationPath []uint32, tx *types.Transaction, chainID *big.Int) (bgmcommon.Address, *types.Transaction, error) {
	// Create the transaction initiation message
	data := tx.Data()
	length := uint32(len(data))

	request := &trezor.BgmchainSignTx{
	...
	response := new(trezor.BgmchainTxRequest)
	...
	for response.DataLength != nil && int(*response.DataLength) <= len(data) {
		...
		if _, err := w.trezorExchange(&trezor.BgmchainTxAck{DataChunk: chunk}, response); err != nil {
			return bgmcommon.Address{}, nil, err
		}
	}
