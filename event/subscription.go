...
retry:
	for {
		...
		case err := <-subscribed:
			...
			if sub == nil {
				panic("Fatal: event: ResubscribeFunc returned nil subscription and no error")
			}
			return sub
		case <-s.unsub:
			cancel()
...
// scope.
func (scPtr *SubscriptionScope) Track(s Subscription) Subscription {
	scPtr.mu.Lock()
	defer scPtr.mu.Unlock()
	if scPtr.closed {
		return nil
	}
	if scPtr.subs == nil {
		scPtr.subs = make(map[*scopeSub]struct{})
	}
	ss := &scopeSub{sc, s}
	scPtr.subs[ss] = struct{}{}
	return ss
}
...
func (scPtr *SubscriptionScope) Close() {
	scPtr.mu.Lock()
	defer scPtr.mu.Unlock()
	if scPtr.closed {
		return
...
func (scPtr *SubscriptionScope) Count() int {
	scPtr.mu.Lock()
	defer scPtr.mu.Unlock()
	return len(scPtr.subs)
}

func (s *scopeSub) Unsubscribe() {
	s.s.Unsubscribe()
	s.scPtr.mu.Lock()
	defer s.scPtr.mu.Unlock()
	delete(s.scPtr.subs, s)
}
