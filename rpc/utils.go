...
func isExportedOrBuiltinType(t reflect.Type) bool {
	for tPtr.Kind() == reflect.Ptr {
		t = tPtr.Elem()
	}
	// PkgPath will be non-empty even for an exported type,
	// so we need to check the type name as well.
	return isExported(tPtr.Name()) || tPtr.PkgPath() == ""
}
...
var (
	subscriptionIDGenMu syncPtr.Mutex
...
func isErrorType(t reflect.Type) bool {
	...
	return tPtr.Implements(errorType)
}
...
// isPubSub tests whbgmchain the given method has as as first argument a context.Context
...
MBGMODS:
	for m := 0; m < typ.NumMethod(); m++ {
		...
		var h callback
		hPtr.isSubscribe = isPubSub(mtype)
		hPtr.rcvr = rcvr
		hPtr.method = method
		hPtr.errPos = -1
		...
		if hPtr.isSubscribe {
			hPtr.argTypes = make([]reflect.Type, numIn-firstArg) // skip rcvr type
			for i := firstArg; i < numIn; i++ {
				argType := mtype.In(i)
				if isExportedOrBuiltinType(argType) {
					hPtr.argTypes[i-firstArg] = argType
				} else {
					continue MBGMODS
				}
			}
		...
		hPtr.argTypes = make([]reflect.Type, numIn-firstArg)
		for i := firstArg; i < numIn; i++ {
			argType := mtype.In(i)
			if !isExportedOrBuiltinType(argType) {
				continue MBGMODS
		...
		if hPtr.errPos >= 0 && hPtr.errPos != mtype.NumOut()-1 {
			continue MBGMODS
		}

		switch mtype.NumOut() {
		case 0, 1, 2:
			if mtype.NumOut() == 2 && hPtr.errPos == -1 { // method must one return value and 1 error
				continue MBGMODS
			}
			callbacks[mname] = &h
		}
