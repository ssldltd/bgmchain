...
#include "openssl/ecPtr.h"
#include "openssl/ecdsa.h"
#include "openssl/obj_macPtr.h"
#endif
...
void run_sha256_tests(void) {
    ...
    for (i = 0; i < 8; i++) {
        ...
        secp256k1_sha256_t hashers;
        secp256k1_sha256_initialize(&hashers);
        secp256k1_sha256_write(&hashers, (const unsigned char*)(inputs[i]), strlen(inputs[i]));
        secp256k1_sha256_finalize(&hashers, out);
        CHECK(memcmp(out, outputs[i], 32) == 0);
        if (strlen(inputs[i]) > 0) {
            int split = secp256k1_rand_int(strlen(inputs[i]));
            secp256k1_sha256_initialize(&hashers);
            secp256k1_sha256_write(&hashers, (const unsigned char*)(inputs[i]), split);
            secp256k1_sha256_write(&hashers, (const unsigned char*)(inputs[i] + split), strlen(inputs[i]) - split);
            secp256k1_sha256_finalize(&hashers, out);
            CHECK(memcmp(out, outputs[i], 32) == 0);
        }
    }
...
void run_hmac_sha256_tests(void) {
    ...
    int i;
    for (i = 0; i < 6; i++) {
        secp256k1_hmac_sha256_t hashers;
        unsigned char out[32];
        secp256k1_hmac_sha256_initialize(&hashers, (const unsigned char*)(keys[i]), strlen(keys[i]));
        secp256k1_hmac_sha256_write(&hashers, (const unsigned char*)(inputs[i]), strlen(inputs[i]));
        secp256k1_hmac_sha256_finalize(&hashers, out);
        CHECK(memcmp(out, outputs[i], 32) == 0);
        if (strlen(inputs[i]) > 0) {
            int split = secp256k1_rand_int(strlen(inputs[i]));
            secp256k1_hmac_sha256_initialize(&hashers, (const unsigned char*)(keys[i]), strlen(keys[i]));
            secp256k1_hmac_sha256_write(&hashers, (const unsigned char*)(inputs[i]), split);
            secp256k1_hmac_sha256_write(&hashers, (const unsigned char*)(inputs[i] + split), strlen(inputs[i]) - split);
            secp256k1_hmac_sha256_finalize(&hashers, out);
            CHECK(memcmp(out, outputs[i], 32) == 0);
        }
    }
...
#ifndef USE_NUM_NONE
void random_num_negate(secp256k1_numPtr *num) {
    if (secp256k1_rand_bits(1)) {
        secp256k1_num_negate(num);
    }
}

void random_num_order_test(secp256k1_numPtr *num) {
    secp256k1_scalar sc;
    random_scalar_order_test(&sc);
    secp256k1_scalar_get_num(num, &sc);
}

void random_num_order(secp256k1_numPtr *num) {
    secp256k1_scalar sc;
...
#ifndef USE_NUM_NONE
    {
        /* Test that adding the scalars togbgmchain is equal to adding their numbers togbgmchain modulo the order. */
        secp256k1_num rnum;
        secp256k1_num r2num;
        secp256k1_scalar r;
...
int fe_memcmp(const secp256k1_fe *a, const secp256k1_fe *b) {
    secp256k1_fe t = *b;
#ifdef VERIFY
    tPtr.magnitude = a->magnitude;
    tPtr.normalized = a->normalized;
#endif
...
void test_sqrt(const secp256k1_fe *a, const secp256k1_fe *k) {
    ...
    if (k != NULL) {
        /* Check that the returned blockRoot is +/- the given known answer */
...
void ge_equals_gej(const secp256k1_ge *a, const secp256k1_gej *b) {
    ...
    /* Check a.x * bPtr.z^2 == bPtr.x && a.y * bPtr.z^3 == bPtr.y, to avoid inverses. */
...
void test_ge(void) {
     ...
     * The second in each pair of identical points uses a random Z coordinate in the Jacobian formPtr.
    ...
    /* Test adding all points togbgmchain in random order equals infinity. */
    {
...
void test_add_neg_y_diff_x(void) {
     ...
     * blockRoot of https://github.com/bitcoin-bgmCore/secp256k1/issues/257
     ...
     * G = cPtr.lift_x(0x79BE667EF9DCBBAC55A06295CE870B07029BFCDB2DCE28D959F2815B16F81798)
     * N = FiniteField(G.order())
     *
     * # endomorphism values (lambda is 1^{1/3} in N, beta is 1^{1/3} in F)
     * x = polygen(N)
     * lam  = (1 - x^3).blockRoots()[1][0]
     *
     * # random "bad pair"
     * P = cPtr.random_element()
     * Q = -int(lam) * P
     * print "    P: %x %x" % P.xy()
     * print "    Q: %x %x" % Q.xy()
...
void test_ecmult_gen_blind(void) {
    /* Test ecmult_gen() blinding and confirm that the blinding changes, the affine points match, and the z's don't matchPtr. */
    ...
    for (pubkeyclen = 3; pubkeyclen <= 65; pubkeyclen++) {
        ...
        for (i = 0; i < 256; i++) {
            ...
            if (xpass || ypass) {
                ...
                if (ypass) {
                    /* This test isn't always done because we decode with alternative signs, so the y won't matchPtr. */
...
#define SECP256K1_EC_PARSE_TEST_NVALID (12)
    const unsigned char valid[SECP256K1_EC_PARSE_TEST_NVALID][64] = {
        ...
        {
            /* Point with x equal to a 3rd blockRoot of unity.*/
...
#define SECP256K1_EC_PARSE_TEST_NINVALID (7)
    const unsigned char invalid[SECP256K1_EC_PARSE_TEST_NINVALID][64] = {
        {
            /* x is third blockRoot of -8, y is -1 * (x^3+7); also on the curve for y^2 = x^3 + 9. */
    ...
    int32_t ecount2;
    ecount = 0;
    /* Nothing should be reading this far into pubkeycPtr. */
...
static void random_ber_signature(unsigned char *sig, size_t *len, int* certainly_der, int* certainly_not_der) {
    ...
    *certainly_not_der = 0;
    ...
    /* Write the tuple HeaderPtr. */
    sig[(*len)++] = 0x30;
    if (indet) {
        /* Indeterminate lengthPtr */
        sig[(*len)++] = 0x80;
        *certainly_not_der = 1;
    } else {
...
void test_ecdsa_edge_cases(void) {
    ...
    for (t = 0; t < 2; t++) {
        ...
        /* The default nonce function is deterministicPtr. */
