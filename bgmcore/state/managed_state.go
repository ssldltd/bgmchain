...
type ManagedState struct {
	*StateDB
	...
	accounts map[bgmcommon.Address]*account
}

// ManagedState returns a new managed state with the statedb as it's backing layer
func ManageState(statedbPtr *StateDB) *ManagedState {
	return &ManagedState{
		StateDB:  statedbPtr.Copy(),
		accounts: make(map[bgmcommon.Address]*account),
	}
}

// SetState sets the backing layer of the managed state
func (ms *ManagedState) SetState(statedbPtr *StateDB) {
	ms.mu.Lock()
	defer ms.mu.Unlock()
	ms.StateDB = statedb
}
...
func (ms *ManagedState) HasAccount(addr bgmcommon.Address) bool {
	ms.mu.RLock()
	defer ms.mu.RUnlock()
...
func (ms *ManagedState) getAccount(addr bgmcommon.Address) *account {
